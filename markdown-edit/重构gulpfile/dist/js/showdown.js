    ...
    this.makeMarkdown = function (src) {
      ...
      function parseNode(node, spansOnly) {
        ...
        switch (tagName) {
          ...
          case 'h1':
            if (!spansOnly) {
              txt = parseHeader(node, 1) + '\n\n';
            }
            break;
          case 'h2':
            if (!spansOnly) {
              txt = parseHeader(node, 2) + '\n\n';
            }
            break;
          case 'h3':
            if (!spansOnly) {
              txt = parseHeader(node, 3) + '\n\n';
            }
            break;
          case 'h4':
            if (!spansOnly) {
              txt = parseHeader(node, 4) + '\n\n';
            }
            break;
          case 'h5':
            if (!spansOnly) {
              txt = parseHeader(node, 5) + '\n\n';
            }
            break;
          case 'h6':
            if (!spansOnly) {
              txt = parseHeader(node, 6) + '\n\n';
            }
            break;

          case 'p':
            if (!spansOnly) {
              txt = parseParagraph(node) + '\n\n';
            }
            break;

          case 'blockquote':
            if (!spansOnly) {
              txt = parseBlockquote(node) + '\n\n';
            }
            break;
          ...
          case 'ol':
            if (!spansOnly) {
              txt = parseList(node, 'ol') + '\n\n';
            }
            break;

          case 'ul':
            if (!spansOnly) {
              txt = parseList(node, 'ul') + '\n\n';
            }
            break;

          case 'precode':
            if (!spansOnly) {
              txt = parsePreCode(node) + '\n\n';
            }
            break;
